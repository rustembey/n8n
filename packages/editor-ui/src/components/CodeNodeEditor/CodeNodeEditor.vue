			...
			<template #customContent>
				<div :class="$style.accordionContent">
					<div :class="$style.aiContent">
						...
						<n8n-input v-model="apiKey" type="password" :rows="5" ref="input" placeholder="API Key"/>
						...
						<div :class="$style.controls">
							<n8n-button :loading="isGenerating" label="Generate" @click="generateCode" :disabled="!apiKey" />
						</div>
					</div>
				...
				body: JSON.stringify({
					...
					messages: [
						...
						},
						{
							role: 'system',
							content:
								`The input follows following schema. You can reference the properties if it's required for the generation. You can reference the data via \`$input.all()\` which will return array of all the items. To access individual item you would use for example, \`$input.all()[0].json\` notation to get the zeroth index-item in the array. You always need to return the result. Schema: \n ${window.__schema ? JSON.stringify(window.__schema.value) : "No schema, this is a first node in the workflow."}`,
						},
						{
							role: 'user',
